...
export const StatsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateUserProfile = useCallback(async (name: string, avatarUrl?: string) => {
        // 1. Update DB
        await UserStatsService.updateProfileData(name, avatarUrl);
        ...
        // 3. Refresh local stats
        await refreshStats();

        // 4. Ideally, we should also call auth.updateProfile if we want valid metadata.
        // I'll assume the component calling this might call both, OR I'll pull it in.
    }, [refreshStats, updateProfile]);
